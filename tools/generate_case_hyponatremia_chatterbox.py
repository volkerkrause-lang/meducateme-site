...
NARRATION=ROOT/'narration'/'case-hyponatremia.json'
OUT=ROOT/'audio'/'case-hyponatremia-chatterbox'
...
for stage in stages:
    rendered=[]; chunks=split_script(stage['script'])
    for i,chunk in enumerate(chunks):
        wav=model.generate(chunk,audio_prompt_path=str(REFERENCE),exaggeration=EXAGGERATION,cfg_weight=CFG_WEIGHT).cpu();rendered.append(wav)
        if i<len(chunks)-1: rendered.append(torch.zeros((wav.shape[0],int(model.sr*PAUSE_MS/1000)),dtype=wav.dtype))
    combined=torch.cat(rendered,dim=-1); wav_path=OUT/f"{stage['id']}.wav"; mp3_path=OUT/f"{stage['id']}.mp3"
