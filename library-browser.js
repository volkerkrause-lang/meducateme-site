(() => {
  ...
  const lessonCatalogue=window.MeducateMeLessons;
  if(!lessonCatalogue) return;
  const previewId=(section,group,title)=>['preview',section,group,title].join('-').toLowerCase().normalize('NFKD').replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'');
  ...
  Object.entries(data.groups).forEach(([group,items],gi)=>{
    ...
    items.forEach(raw=>{const [t,sub]=raw.split(' — ');const lesson=lessonCatalogue.byListing(key,t);const href=lesson?.href||null;const live=!!href;const access=lesson?.access||'preview';const id=lesson?.id||previewId(key,group,t);const b=document.createElement('button');b.type='button';b.className='topic-item'+(live?' live':'');b.dataset.search=(group+' '+raw).toLowerCase();b.dataset.title=t;b.dataset.group=group;b.dataset.desc=sub||'';b.dataset.live=live?'1':'0';b.dataset.href=href||'';b.dataset.lessonId=id;b.dataset.access=access;b.innerHTML=`<span><strong>${t}</strong>${sub?`<small>${sub}</small>`:''}</span><em>${live?'Live':'Preview'}</em>`;box.appendChild(b);flat.push(b)});
    list.appendChild(d)
  });
  ...
  function showPreview(opts){
    ...
    shell.dataset.lessonId=opts.id||'';shell.dataset.access=opts.access||'preview';
  ...
  const flagship=lessonCatalogue.byHref(data.flag.href);
  showPreview({title:data.flag.title,desc:data.flag.desc,meta:config.flagLabel,live:true,href:data.flag.href,id:flagship?.id,access:flagship?.access});
  list.addEventListener('click',e=>{const b=e.target.closest('.topic-item');if(!b)return;const live=b.dataset.live==='1';showPreview({el:b,title:b.dataset.title,desc:b.dataset.desc||(key==='cases'?'Work through this presentation step by step, then connect the clinical clues back to the underlying mechanism.':'Build this topic from first principles, then connect the mechanism to clinical findings and decisions.'),meta:b.dataset.group,live,href:live?b.dataset.href:null,id:b.dataset.lessonId,access:b.dataset.access})});
